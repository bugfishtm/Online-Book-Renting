<?php
	...
	*/	 if(!is_array(@$object)) { @http_response_code(404); Header("Location ../"); exit(); }
	...
	$object["add_head_ext"] 				= '<link rel="icon" type="image/x-icon" href="'._HIVE_URL_REL_.'/_site/'._HIVE_MODE_.'/favicon.ico">';
	...
	switch(_HIVE_URL_CUR_[0]) {
		...
		case "profile": 
			...
			break;		
		case "deadlines": 
			if($object["user"]->loggedIn AND ($object["user"]->user["user_initial"] OR hive__access($object, "page_deadlines", false))) {
		...
		case "view": 
			...
			break;	
		case "tag": 
			if($object["user"]->loggedIn AND ($object["user"]->user["user_initial"] OR hive__access($object, "page_tag", false))) {
			...
			break;	
		case "user": 
			if($object["user"]->loggedIn AND ($object["user"]->user["user_initial"] OR hive__access($object, "page_user", false))) {
			...
			break;	
		case "blacklist": 
			if($object["user"]->loggedIn AND ($object["user"]->user["user_initial"] OR hive__access($object, "page_blacklist", false))) {
			...
			break;	
		case "logging": 
			if($object["user"]->loggedIn AND ($object["user"]->user["user_initial"] OR hive__access($object, "page_logging", false))) {
			...
			break;				
		case "settings": 
			if($object["user"]->loggedIn AND ($object["user"]->user["user_initial"] OR hive__access($object, "page_settings", false))) {
