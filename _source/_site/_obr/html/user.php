	...
	if(!is_array($object)) { @http_response_code(404); Header("Location: ../"); exit(); } 
		...
		if(is_array($value_array)) {
			foreach($value_array as $key => $value) {
				...
				$value_array[$key]["user_firstname"] = $value_array[$key]["user_firstname"]." ".$value_array[$key]["user_lastname"];
				if(@trim($value_array[$key]["user_firstname"] ?? '') == "") { $value_array[$key]["user_firstname"] = "- - -"; }

				$value_array[$key]["user_lastname"] = $value_array[$key]["user_street"]." ";
				if(@trim($value_array[$key]["user_postcode"] ?? '') != "") { $value_array[$key]["user_lastname"] .= "<br />".$value_array[$key]["user_postcode"]; }
				if(@trim($value_array[$key]["user_postcode"] ?? '') == "" AND @trim($value_array[$key]["user_region"] ?? '') != "") { $value_array[$key]["user_lastname"] .= "<br />".$value_array[$key]["user_region"]; }
				if(@trim($value_array[$key]["user_postcode"] ?? '') != "" AND @trim($value_array[$key]["user_region"] ?? '') != "") { $value_array[$key]["user_lastname"] .= " ".$value_array[$key]["user_region"]; }			
				if(@trim($value_array[$key]["user_country"] ?? '') != "") { $value_array[$key]["user_lastname"] .= "<br />".$value_array[$key]["user_country"]; }		
